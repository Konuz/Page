...
document.addEventListener('DOMContentLoaded', function() {
                ...
                category.subcategories.forEach(subcategory => {
                    subcategory.tools.forEach(tool => {
                        toolsData.push({
                            ...
                            searchText: `${tool.name} ${category.category} ${subcategory.name}`.toLowerCase()
                        });
                    });
                });
            });
    ...
    function performSearch(query) {
        ...
        const results = toolsData
            .map(tool => ({
                ...tool,
                score: fuzzySearch(query, tool.searchText)
            }))
    ...
    function displayResults(results, query) {
        if (results.length === 0) {
            searchResults.innerHTML = '<div class="search-no-results">Nie znaleziono narzędzi</div>';
            searchResults.classList.add('active');
            return;
        }

        const html = results.map((tool, index) => `
            <div class="search-result-item" data-index="${index}" data-tool-id="${tool.id}">
                <img src="${tool.image}" alt="${tool.name}" class="search-result-image" loading="lazy">
                <div class="search-result-info">
                    <div class="search-result-title">${highlightText(tool.name, query)}</div>
                    <div class="search-result-category">${tool.category} › ${tool.subcategory}</div>
                </div>
            </div>
        `).join('');
    ...
    function navigateToTool(toolId) {
        // Find the tool and its category info
    ...
    function handleKeyboardNavigation(e) {
        ...
        if (e.key === 'ArrowDown') {
            ...
            if (currentHighlighted >= 0 && items[currentHighlighted]) {
                const toolId = items[currentHighlighted].dataset.toolId;
                navigateToTool(toolId);
            }
